...
public class Bullet : MonoBehaviour
{
    ...
    public void Instantited(GameObject whoShot, Vector3 direction)
    {
        _whoShot = whoShot;

        _rigidbody.AddForce(direction * Random.Range(_bulletImpulse * 0.95f, _bulletTime * 1.05f), ForceMode2D.Impulse);
    ...
    {
        if (_whoShot == collision.gameObject) return;
        ...
        else if (collision.gameObject.CompareTag("Enemy"))
        {
            ...
            collision.gameObject.GetComponentInParent<Enemy>().GotHit(damageFactor, transform.position, _whoShot.layer == 6);
            DestroyBullet();
        }
